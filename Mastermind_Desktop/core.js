...
const TOOL_GROUPS = {
    CORE: ["skill_benchmark_runner", "skill_benchmark_evolution", "skill_manager", "skill_synthesis", "architecture_loop", "runtime_verify", "read_file", "write_file", "replace_file_content", "multi_replace_file_content", "thought", "repairSystem", "request_tool_discovery", "run_terminal_command", "list_local_files", "list_files", "analyze_file", "fast_file_search", "discovery_scan"],
...
async function runToolLoop(history) {
    ...
    try {
        ...
        toolResult = { error: "Universal agent operating loop failed.", details: e?.message || String(e) };
                ...
                else if (name === "skill_deep_understanding") toolResult = await universalSkillDeepUnderstandingManager(args?.action || "synthesize", args || {}, {
 ...
 * Shared semantic model, concept graph, procedure extraction and conflict analysis.
...
function usduRelations(units=[]){const rel=[],seen=new Set();for(const a of usduArray(units)){const aText=[...usduArray(a.headings),...usduArray(a.rules),...usduArray(a.codeSignals)].join(' ').toLowerCase();const at=usduTokens(aText);for(const b of usduArray(units)){if(a.path===b.path)continue;const bt=new Set(usduTokens([b.path,...usduArray(b.headings),...usduArray(b.rules)].join(' ')));const overlap=at.filter(t=>bt.has(t));if(overlap.length<3)continue;const key=[a.path,b.path].sort().join('|');if(seen.has(key))continue;seen.add(key);rel.push({from:a.path,to:b.path,sharedConcepts:overlap.slice(0,12)})}}return rel.slice(0,250)}
function usduConflicts(units=[]){const pos=[],neg=[];for(const u of usduArray(units)){for(const r of usduArray(u.rules)){if(/\b(always|must|required|يجب|ينبغي)\b/i.test(r))pos.push({path:u.path,rule:r});if(/\b(never|must not|required not|do not|تجنب|لا يجب)\b/i.test(r))neg.push({path:u.path,rule:r})}}const conflicts=[];for(const p of pos){const pk=usduTokens(p.rule).filter(x=>x.length>4);for(const n of neg){const nk=usduTokens(n.rule).filter(x=>x.length>4),shared=pk.filter(x=>nk.includes(x));if(shared.length>=2)conflicts.push({positive:p,negative:n,sharedTerms:shared})}}return conflicts.slice(0,100)}
...
function usduBuildModel(input={}){const units=usduArray(input.units||input.sources||[]),concepts=usduConcepts(units),relations=usduRelations(units),conflicts=usduConflicts(units),procedures=usduProcedures(units),evidence=usduEvidenceMap(units),domain=usduStr(input.domain||'general'),topic=usduStr(input.topic||input.name||'Expert Domain'),confidence=units.length?Math.min(.95,.35+Math.min(.45,units.length*.02)+Math.min(.15,concepts.length*.001)):0;return{version:UNIVERSAL_SKILL_DEEP_UNDERSTANDING_VERSION,domain,topic,sourceCount:units.length,concepts,relations,procedures,conflicts,evidence,confidence,limitations:conflicts.length?'Potential source conflicts detected; resolve before activation.':'Semantic model remains evidence-aware; source quality and runtime verification still matter.'}}
function usduSynthesize(input={}){const model=usduBuildModel(input),top=model.concepts.slice(0,30).map(x=>x.term),instructions=[`Build task decisions from the Deep Understanding model for ${model.topic}.`,`Prefer source-backed procedures and cite provenance when making expert claims.`,`When source rules conflict, do not silently choose; surface the conflict and seek evidence.`,...model.procedures.slice(0,25).map(p=>p.statement)],workflow=[{id:'understand',action:'load_semantic_model',purpose:'Use concepts, relations, procedures and conflicts before applying the Skill.',tool:'skill_deep_context',expectedEvidence:['semantic model','source provenance'],stopConditions:['no source evidence']}];if(model.conflicts.length)workflow.push({id:'conflict-resolution',action:'resolve_source_conflicts',purpose:'Resolve contradictory directives before execution.',tool:'evidence_engine',expectedEvidence:['conflict resolution evidence'],stopConditions:['unresolved high-risk conflict']});workflow.push({id:'apply',action:'apply_expert_procedure',purpose:'Apply domain-specific procedure using the semantic model.',tool:'skill_context',inputs:{concepts:top},expectedEvidence:['task outcome evidence'],stopConditions:['evidence contradicts source']});const skill={id:usduStr(input.id||`skill.deep.${String(input.topic||'expert').toLowerCase().replace(/[^a-z0-9]+/g,'.')}`),name:usduStr(input.name||`Deep Expert Skill: ${model.topic}`),version:'0.2.0',description:usduStr(input.description||`Deep-understanding expert Skill for ${model.topic} derived from ${model.sourceCount} source units.`),domain:model.domain,triggers:top.slice(0,15),tags:[model.domain,'deep-understanding','evidence-driven'],instructions,workflow,capabilities:top.slice(0,20).map((x,i)=>({id:`deep.cap.${i+1}`,name:x,description:'Concept identified in source evidence.',tools:['skill_deep_context','read_file'],safe:true})),constraints:['Preserve source provenance.','Do not silently resolve conflicting source rules.','Do not execute imported source code.'],validation:{required:['semantic model generated','provenance available','conflicts reviewed'],commands:[],evidence:['concept graph','procedure map','conflict report','source evidence']},outputs:['deep skill definition','semantic model','conflict report','evidence map'],safety:{allowNetwork:false,allowWrites:false,allowTerminal:false,allowSecrets:false,arbitraryCode:false},provenance:{sourceType:usduStr(input.sourceType||'deep-synthesized'),repository:usduStr(input.repository),path:usduStr(input.path),ref:usduStr(input.ref||'main')},status:'draft'};return{skill,understanding:model}}
 ...
 * Desktop counterpart of the declarative Skill benchmark/evolution layer.
 * Imported source/Skill code is never executed by this engine.
 */
const UNIVERSAL_SKILL_BENCHMARK_EVOLUTION_VERSION='1.0-benchmark-evolution';
function usbeArray(v){return Array.isArray(v)?v:[];}function usbeObj(v){return v&&typeof v==='object'&&!Array.isArray(v)?v:{};}function usbeStr(v){return typeof v==='string'?v:'';}function usbeId(v,fallback='case'){return usbeStr(v).toLowerCase().replace(/[^a-z0-9._-]+/g,'-').replace(/^-+|-+$/g,'')||fallback;}function usbeTokens(text=''){return [...new Set(usbeStr(text).toLowerCase().match(/[a-z0-9_\u0600-\u06ff-]{2,}/gi)||[])];}function usbeOverlap(a=[],b=[]){const bs=new Set(usbeTokens(b.join(' ')));return usbeTokens(a.join(' ')).filter(x=>bs.has(x));}
function usbeDefaultCases(skill={}){const caps=usbeArray(skill.capabilities),validation=usbeObj(skill.validation),concepts=[...usbeArray(skill.triggers),...usbeArray(skill.tags),...caps.map(x=>usbeStr(x.name))].filter(Boolean).slice(0,12),cases=[];cases.push({id:'contract-schema',name:'Skill contract integrity',type:'structural',input:{skillId:skill.id},expected:{requiredFields:['id','name','version','instructions','workflow','validation','safety']},evaluation:{mode:'required_fields',dimension:'contract'}});cases.push({id:'evidence-gate',name:'Evidence requirements are actionable',type:'structural',input:{validation},expected:{minEvidence:Math.max(1,usbeArray(validation.evidence).length)},evaluation:{mode:'evidence_requirements',dimension:'evidence'}});cases.push({id:'safety-boundary',name:'Safety boundary preservation',type:'structural',input:{safety:usbeObj(skill.safety)},expected:{forbiddenTrue:['arbitraryCode','allowSecrets']},evaluation:{mode:'safety_flags',dimension:'safety'}});if(concepts.length)cases.push({id:'concept-coverage',name:'Domain trigger coverage',type:'structural',input:{concepts},expected:{minConcepts:Math.min(3,concepts.length)},evaluation:{mode:'concept_coverage',dimension:'coverage'}});return cases;}
function usbeGenerateCases(skill={},input={}){const supplied=usbeArray(input.cases);if(supplied.length)return supplied.map((c,i)=>({...usbeObj(c),id:usbeId(c.id,`case-${i+1}`)}));return usbeDefaultCases(skill);}
function usbeRequiredFieldCheck(skill,required=[]){return required.every(k=>k.split('.').reduce((o,p)=>o?.[p],skill)!==undefined);}function usbeEvaluateCase(skill,c,result={}){const expected=usbeObj(c.expected),evaluation=usbeObj(c.evaluation),mode=evaluation.mode||'result';let passed=false,score=0,evidence=[],weakness='';if(mode==='required_fields'){const required=usbeArray(expected.requiredFields);passed=usbeRequiredFieldCheck(skill,required);score=passed?1:required.filter(k=>k.split('.').reduce((o,p)=>o?.[p],skill)!==undefined).length/Math.max(1,required.length);evidence.push(`required_fields:${required.join(',')}`);if(!passed)weakness='missing required Skill contract fields';}else if(mode==='evidence_requirements'){const actual=usbeArray(skill?.validation?.evidence),min=Number(expected.minEvidence||1);passed=actual.length>=min;score=Math.min(1,actual.length/Math.max(1,min));evidence.push(`validation.evidence.count=${actual.length}`);if(!passed)weakness='insufficient evidence requirements';}else if(mode==='safety_flags'){const safety=usbeObj(skill.safety),forbidden=usbeArray(expected.forbiddenTrue);passed=forbidden.every(k=>safety[k]!==true);score=passed?1:forbidden.filter(k=>safety[k]!==true).length/Math.max(1,forbidden.length);evidence.push(`safe:${JSON.stringify(safety)}`);if(!passed)weakness='unsafe capability flag enabled';}else if(mode==='concept_coverage'){const concepts=usbeArray(expected.concepts||c.input?.concepts),pool=[...usbeArray(skill.triggers),...usbeArray(skill.tags),...usbeArray(skill.capabilities).map(x=>x.name)],overlap=usbeOverlap(concepts,pool),min=Number(expected.minConcepts||1);passed=overlap.length>=min;score=Math.min(1,overlap.length/Math.max(1,min));evidence.push(`concept_overlap:${overlap.slice(0,20).join(',')}`);if(!passed)weakness='benchmark concepts not sufficiently represented by the Skill';}else{const supplied=usbeObj(result),expectedText=usbeArray(expected.contains),hay=JSON.stringify(supplied).toLowerCase(),hits=expectedText.filter(x=>hay.includes(usbeStr(x).toLowerCase()));passed=expectedText.length?hits.length===expectedText.length:Boolean(supplied.passed);score=expectedText.length?hits.length/expectedText.length:(passed?1:0);evidence.push('external_result_supplied');if(!passed)weakness=usbeStr(supplied.weakness||'benchmark execution did not meet the expected evidence');}return{id:c.id,name:c.name,passed,score:Number(score.toFixed(3)),dimension:evaluation.dimension||'general',evidence,weakness};}
function usbeAggregate(results=[]){const rs=usbeArray(results),by={};for(const r of rs){const d=r.dimension||'general';(by[d]||(by[d]=[])).push(r.score);}const dimensions=Object.fromEntries(Object.entries(by).map(([d,v])=>[d,Number((v.reduce((a,b)=>a+b,0)/Math.max(1,v.length)).toFixed(3))])),score=Number((rs.reduce((a,b)=>a+b.score,0)/Math.max(1,rs.length)).toFixed(3)),failures=rs.filter(r=>!r.passed);return{score,dimensions,passed:failures.length===0,caseCount:rs.length,failedCaseIds:failures.map(x=>x.id),weaknesses:[...new Set(failures.map(x=>x.weakness).filter(Boolean))]};}
function usbeRun(skill={},cases=[],executionResults={}){const results=usbeArray(cases).map(c=>usbeEvaluateCase(skill,c,executionResults[c.id]||{})),aggregate=usbeAggregate(results);return{version:UNIVERSAL_SKILL_BENCHMARK_EVOLUTION_VERSION,skillId:skill.id||'unknown',results,aggregate,evidence:{benchmarkCases:cases.map(c=>c.id),evaluatedAt:new Date().toISOString(),executionResultsProvided:Object.keys(executionResults).length>0},verdict:aggregate.passed?'accepted':aggregate.score>=0.6?'partial/needs_review':'failed'};}
function usbeBuildEvolutionPlan(skill={},benchmark={}){const weak=usbeArray(benchmark?.aggregate?.weaknesses),dims=usbeObj(benchmark?.aggregate?.dimensions),instructions=[...usbeArray(skill.instructions)],constraints=[...usbeArray(skill.constraints)],validation=usbeObj(skill.validation),changes=[];if(weak.length)for(const w of weak.slice(0,8)){const text=`Improve against benchmark weakness: ${w}`;if(!instructions.includes(text))instructions.push(text);changes.push({type:'instruction',reason:w});}if((dims.evidence||1)<0.8){const ev=usbeArray(validation.evidence);if(!ev.includes('benchmark outcome evidence'))ev.push('benchmark outcome evidence');validation.evidence=ev;changes.push({type:'validation',reason:'evidence score below target'});}if((dims.safety||1)<1){constraints.push('Preserve strict safety flags: no secrets and no arbitrary code execution.');changes.push({type:'safety_constraint',reason:'safety benchmark weakness'});}return{changes,instructions,constraints,validation,targetScore:0.85,minimumScoreDelta:0.05};}
function usbeEvolve(skill={},benchmark={}){const plan=usbeBuildEvolutionPlan(skill,benchmark),prior=usbeStr(skill.version||'0.1.0').split('.').map(x=>parseInt(x,10)||0);prior[1]=(prior[1]||0)+1;prior[2]=0;const next={...skill,version:prior.join('.'),instructions:plan.instructions,constraints:[...new Set(plan.constraints)],validation:plan.validation,status:'draft',evolution:{fromVersion:skill.version||'0.1.0',benchmarkScore:benchmark?.aggregate?.score||0,benchmarkVerdict:benchmark?.verdict||'unknown',changes:plan.changes,targetScore:plan.targetScore}};return{skill:next,plan,reason:benchmark?.aggregate?.passed?'preventive refinement':'repair based on benchmark weaknesses'};}
function usbeCompare(prev={},next={}){const a=usbeObj(prev),b=usbeObj(next),scoreDelta=Number(((b.aggregate?.score||0)-(a.aggregate?.score||0)).toFixed(3));return{previousScore:a.aggregate?.score||0,nextScore:b.aggregate?.score||0,scoreDelta,improved:scoreDelta>0,noRegression:scoreDelta>=0,previousVersion:a.version,nextVersion:b.version};}
function universalSkillBenchmarkEvolutionManager(action,args={},adapter={}){const a=String(action||'').toLowerCase(),skill=usbeObj(args.skill);if(a==='generate'||a==='build_cases'||a==='create_cases')return{version:UNIVERSAL_SKILL_BENCHMARK_EVOLUTION_VERSION,skillId:skill.id,cases:usbeGenerateCases(skill,args)};if(a==='run'||a==='benchmark'||a==='test'){const cases=usbeGenerateCases(skill,args),executionResults=usbeObj(args.executionResults);if(typeof adapter.executeCase==='function'&&args.execute===true)return Promise.resolve(adapter.executeCase({skill,cases,args})).then(r=>usbeRun(skill,cases,r||{}));return usbeRun(skill,cases,executionResults);}if(a==='evaluate')return usbeAggregate(usbeArray(args.results));if(a==='evolve'||a==='improve'||a==='repair')return usbeEvolve(skill,args.benchmark||args.result||{});if(a==='compare')return usbeCompare(args.previous||{},args.next||{});if(a==='full_cycle'||a==='evolution_cycle'){const cases=usbeGenerateCases(skill,args),benchmark=usbeRun(skill,cases,usbeObj(args.executionResults)),evolution=usbeEvolve(skill,benchmark);return{version:UNIVERSAL_SKILL_BENCHMARK_EVOLUTION_VERSION,skillId:skill.id,cases,benchmark,evolution,workflow:['benchmark generation','skill testing','measurement','weakness analysis','repair/improvement','new draft version'],activationGate:benchmark.aggregate.passed?'explicit activation still required':'retest evolved draft before activation'};}return{ok:false,error:'unknown_benchmark_evolution_action',actions:['generate','build_cases','create_cases','run','benchmark','test','evaluate','evolve','improve','repair','compare','full_cycle','evolution_cycle']};}
...
/**
 * Universal Skill Benchmark Runner 1.0 - Desktop
 * Executes only explicitly declared, allowlisted benchmark commands.
 * Never executes imported Skill/source code.
 */
const UNIVERSAL_SKILL_BENCHMARK_RUNNER_VERSION='1.0-safe-runner';
...
function usbrNormalizeCase(c,i=0){const x=usbrObj(c);return{id:usbrStr(x.id)||`case-${i+1}`,name:usbrStr(x.name)||`Benchmark case ${i+1}`,type:usbrStr(x.type||'structural'),command:usbrStr(x.runner?.command||x.command),timeoutMs:Math.min(120000,Math.max(1000,Number(x.runner?.timeoutMs||30000))),allowCommand:Boolean(x.runner?.allowCommand),expectedExitCodes:usbrArray(x.runner?.expectedExitCodes).length?usbrArray(x.runner.expectedExitCodes).map(Number):[0],expectedOutput:usbrArray(x.runner?.expectedOutput).map(usbrStr)};}
